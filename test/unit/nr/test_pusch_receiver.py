...
class TestPUSCHReceiver:
    ...
    def test_transform_precoding(self):
        """Single-layer transform-precoded PUSCH should decode losslessly."""
        pusch_config = PUSCHConfig()
        pusch_config.n_size_bwp = 273
        pusch_config.tb.mcs_index = 16
        pusch_config.transform_precoding = True
        
        ber = run_test([pusch_config], channel_estimator=None, batch_size=2)
        assert ber == 0.0
