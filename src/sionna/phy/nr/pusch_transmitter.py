...
class PUSCHTransmitter(Block):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        num_lower_guard_carriers = (self._fft_size - self._num_subcarriers) // 2
        num_upper_guard_carriers = num_lower_guard_carriers + \
            self._num_subcarriers - self._num_effective_subcarriers
        self._resource_grid = ResourceGrid(
            ...
            num_guard_carriers=(num_lower_guard_carriers, num_upper_guard_carriers),
        ...
        if self._transform_precoding:
            self._transform_precoder = PUSCHTransformPrecoder(
        ...
        if self._output_domain == "time":
            self._phase_correction_sequence = torch.as_tensor(
                self._phase_correction_sequence,
                dtype=self.cdtype,
                device=self.device,
            ).unsqueeze(-1)
    ...
    def call(
        ...
        if self._ofdm_modulator is not None:
            x = x_pre / self._phase_correction_sequence
