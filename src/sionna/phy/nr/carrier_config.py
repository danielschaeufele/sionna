...
class CarrierConfig(Config):
    ...
    def subcarrier_spacing(self) -> int:
        r"""`int`, (default 15) | 30 | 60 | 120 | 240 | 480 | 960 :
    ...
    def carrier_frequency(self, value: float) -> None:
        if value < 0:
            raise ValueError("carrier_frequency must be not be negative")
    ...
    @property
    def cyclic_prefix_length(self) -> np.ndarray:
        r"""Cyclic prefix length :math:`N_{\text{CP},l}^{\mu} \cdot T_{\text{c}}` [s]."""
