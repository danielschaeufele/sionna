...
from sionna.phy.nr.pusch_transform_precoder import PUSCHTransformDeprecoder
...
class PUSCHReceiver(Block):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if mimo_detector is None:
            transformation = PUSCHTransformDeprecoder(
                pusch_transmitter.resource_grid.num_effective_subcarriers,
                precision=precision
            ) if pusch_transmitter._transform_precoding else None
    ...
    def call(
        ...
        if self._ofdm_demodulator is not None:
            ...
            y = y * self._phase_correction_sequence
